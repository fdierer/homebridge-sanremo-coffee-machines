...
### Changed
- Quieter connectivity logging: transient network blips stay at debug; warn only after 3 consecutive failed poll cycles; log restore only after a warned outage.
- Skip overlapping poll cycles when a previous poll is still in progress.
- Add a 10-second timeout on machine HTTP requests to avoid hung polls.

### Fixed
- Reduced Homebridge log noise and poll pile-up when the Cube briefly fails to respond on the local network.

## [1.4.7] - 2026-02-03

### Added
- Official Homebridge verification: Plugin has been verified by the Homebridge team and meets all verification requirements. The verified badge now appears in the Homebridge UI and plugin search results.
- Donation links support: Added funding field to package.json to enable donation button in Homebridge UI (PayPal).

### Changed
- Updated documentation to reflect verified status and added verification badge to README.
